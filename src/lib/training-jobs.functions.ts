...
export const ingestUrlFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const res = await fetch(data.url, {
      headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120.0.0.0 Safari/537.36", Accept: "text/html,application/xhtml+xml,*/*;q=0.8" },
    ...
    const raw = await cerebrasChat({
      messages: [
        { role: "system", content: `너는 한국 입시 교육 전문가다. 웹 페이지 내용을 분석해 입시 관련 핵심 정보를 추출·정리한다.\n반드시 아래 JSON 형식 하나만 출력. 다른 텍스트 없음.\n{"title":"제목(50자 이내)","content":"핵심 내용 정리(500~2000자)"}` },
    ...
    const { data: doc, error } = await supabaseAdmin.from("training_docs").insert({
      category: data.category, title: parsed?.title ?? data.url, content: parsed?.content ?? rawText.slice(0, 5000), source_type: "url", source_url: data.url,
    }).select("id").single();
    if (error) throw new Error(error.message);
